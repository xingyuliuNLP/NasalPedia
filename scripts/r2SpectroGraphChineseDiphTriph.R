# chinese data
dataChineseManDiphTriph = read.csv("/Users/rosingle/Documents/theseAnalysis/20juin/output/chineseManDiphTriph.csv")
# 
# datawang = reshape(dat1, idvar = "vowel", timevar = "percentage", direction = "wide")
dataChineseManDiphTriph
class(dataChineseManDiphTriph)
names(dataChineseManDiphTriph)[names(dataChineseManDiphTriph) == "number"] <- "percent"
colnames(dataChineseManDiphTriph)
...
trajectoryChineseDiphTriph = dataChineseManDiphTriph %>%
...
ggplot(trajectoryChineseDiphTriph, aes(x = percent, y = hz, group = class, color=class)) +
    ...
    # facet_wrap(~class + vowel, ncol = 5) +
    theme_classic() + ggtitle("Trajectoire formantique des non-monophtongues \n en chinois (locuteur masculin)") + ylab("Fréquence (Hz)") + xlab("Temps (pourcentage)") + 
    theme(plot.title = element_text(hjust = 0.5)) 

ggsave("chineseManDiphTriph.png")
