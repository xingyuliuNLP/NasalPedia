...
rawdata = read.table('emreDiphreform.txt',sep = "\t",header = TRUE)
# View(rawdata)
...
data.melt$Vowel  <- substr(data.melt$file,4,6) %>% as.factor
levels(data.melt$Vowel)
#######################
######## first run to here
IPA_vowels <- c("a", "ɛ",'ɔ','e','ø','i','o','œ','u','y')
IPA_vowels
data.melt$Vowel.IPA <- IPA_vowels[as.numeric(data.melt$Vowel)]
Vowel_space_path <- c("a", "i",'u','y','e','ai','o','ao','eu','oe')
...
save(list = files_to_save, file = "testmyself_diph.RData")